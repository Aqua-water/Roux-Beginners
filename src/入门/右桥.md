...
如果不能通过上面的方法归位，就将绿白棱块转动到M层，并使棱块的白色面朝上，然后再做上述操作。
...
作用是将F、R层交界位置的两个块（灰色）移动到顶层。同时，它们的位置也会被原本在顶层的两个块（黑色）替代。
...
这一步的目标是将绿红棱块放在[起始位](./左桥.md#3-1-将棱块放至起始位)，不关心其朝向。和左桥相同，我们选择DF位作为起始位。
...
首先，将绿红白角块放在顶层。如果它不在顶层，那么一定在右桥位置上（RFD位或RBD位）。此时做一次基本操作即可。在基本操作中，适当地选择转`U`或`U’`，可以避免做完后角块的白色面朝上的情况，从而一步到位。
...
如果绿红白角块已经在顶层，且白色面不朝上，那么需要调整角块的朝向。首先，转`Ux`将角块放在某个[翻转位](./左桥.md#特例2角块位于lfu位且白色面朝上)上。

如果在翻转位UFR位，做基本操作一中的`R U' R'`。
...
> 与复原第一组棱角对不同的是，此时前棱角对已归位，那么应该避免选择基本操作一，因为它会破坏这组棱角对的位置。因此这时翻转位只能选择UBR位，并做基本操作一中的`R U' R'`。同理，如果是先复原后面那组棱角对，那么要避免做基本操作二。
