...
先`Ux`将该棱块放在M层，并且保证其下方不是黄蓝或黄绿棱块。再做[基本操作五](./eo.md#基本操作五-m-u2-m)或[六](#基本操作六-m-u2-m)，完成上下棱块的交换，将其放在底层。

这个例子通过基本操作六，将黄绿棱块移到DB位。
...
这个例子将黄绿棱块移到DB位，然后再通过基本操作五，将黄蓝棱块移到DF位。
...
你也可以尝试：如何不依赖转体，仅使用`U2`和M层操作的组合来复原。
...
之所以笼统地称这类情况为“四心换”，是因为如果做`M2`，那么除了4个中心块外的所有块都已复原。

<div id="L4E-6">
<script type="text/javascript">
  TTk.AlgorithmPuzzle(3)
    .size({width:300, height:300})
    .fc('wwwwywwwwgggggggggrrrrorrrryyyywyyyybbbbbbbbbooooroooo')
    .case(`M2`)
    ('#L4E-6');
</script>
</div>
