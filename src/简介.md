...
本教程采用总分的结构，共有三章（不包括[记号](./记号.md)）。每一章的开头会介绍对应步骤的复原目标，并给出图片形式的简略教程。每章下的小节则会详尽介绍对应步骤的做法。对于有一定基础或者理解能力强的读者，你可以只阅读章前的内容，而跳过小节中的详尽介绍。如果你已经完整学习了一遍，而忘记了某个情形的做法，有时也能通过章节开头的复原指引快速查阅。
