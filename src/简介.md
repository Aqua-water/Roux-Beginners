...
本教程建议使用电脑端阅读，以更方便地观察这些虚拟魔方。如果使用手机端，尽量不要拖动虚拟魔方。

### 复原步骤的阅读

本教程采用总分的结构，共有三章（不包括[记号](./记号.md)）。每一章的开头会介绍对应步骤的复原目标，并给出类似于纸质公式书的简略复原指引。每章下的小节则会详尽介绍对应步骤的做法。对于有一定基础或者理解能力强的读者，你可以只阅读章前的内容，而跳过小节中的详尽介绍。如果你已经完整学习了一遍，而忘记了某个情形的做法，有时也能通过章节开头的复原指引快速查阅。
...
如果某种情形难以通过打乱遇到，你可以使用[反向转动](./入门/记号.md#转动序列的反向)的方法得到这一情形。